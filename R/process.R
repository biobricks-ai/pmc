...
split_list<-function(file_list){
  listCount=length(file_list)/50000
...
parse_txt_data<-function(input_file){
  ...
  output_tibble=tibble(parsed_file) |> mutate(fileid=base_name(input_file)) |> 
    mutate(parsed_front_data=reduce2(c("\n==== Front\n","\n==== Body\n"), c('', ''),  .init = parsed_file, str_replace)) |>
...
combine_txt_data<-function(input_file_list, outputFileName){
  ...
  for( i in seq(1,length(input_file_list))){
    output_data[[i]]=parse_txt_data(input_file_list[i])
  }
...
list_split_body=split_list(file.path(list.files(download_body, pattern = 'txt', full.names = TRUE)))
list_split_front=split_list(file.path(list.files(download_front, pattern = 'txt', full.names = TRUE)))
